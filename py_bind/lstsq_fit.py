...
def gau_least_square(m, n, w, pn, f):
    ...
    return (cm_s, am_s, {"pn": pn, "f": f})

def gau_func(cs_as_n):
...
def gau_abs_err(m, n, pn, f, x1):
    ...
    def __func__(log_ws):
        w = np.exp(log_ws[0])
        res = gau_least_square(m, n, w, pn, f)
        f0 = res[-1]["f"]
        f1 = gau_func(res)
        def diff(x):
...
def gau_least_square_with_opt(m, n, pn, f, w0, x1):
    err = gau_abs_err(m, n, pn, f, x1)
