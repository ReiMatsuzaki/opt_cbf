...
namespace opt_cbf_h {
  ...
  void AlphaGradHess(cV& c, cV& d, cM& U,
    ...
    *g = g2 + g3 - g1;
    ...
    M tmp = Calc_ai_A_Bj_b(r1, U, D10, c);
    M ri_U_Lj_c   = tmp + tmp.transpose();    

    tmp = (r1 * s1.transpose()).array() * U.array();
    M ri_U_sj = tmp + tmp.transpose();

    tmp = Calc_a_Ai_B_Aj_b(d, D10, U, c);
    M d_Li_U_Lj_c = tmp + tmp.transpose();

    tmp = Calc_ai_A_Bj_b(s1, U, D10, d);
    M d_Li_U_sj   = tmp + tmp.transpose();
